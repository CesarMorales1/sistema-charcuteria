...
export class ComprasController {
  constructor(useCases) { Object.assign(this, useCases); }
  ...
  listar = async (req, res, next) => {
    try {
      ...
      const r = await this.listCompras.execute({ page: +page, limit: +limit, id_proveedor, reportable_seniat, estado });
  ...
  obtener = async (req, res, next) => {
    try {
      const c = await this.getCompra.execute(req.params.id);
  ...
  crear = async (req, res, next) => {
    ...
    try {
      const c = await this.createCompra.execute(req.body, req.user.id_usuario);
  ...
  cambiarEstado = async (req, res, next) => {
    try {
      const c = await this.cambiarEstado.execute(req.params.id, req.body.estado);
      res.json({ status: 'success', data: c });
    } catch (e) { this._err(res, next, e); }
  };
  ...
  cancelar = async (req, res, next) => {
    try {
      const c = await this.cancelarCompra.execute(req.params.id);
      res.json({ status: 'success', data: c });
    } catch (e) { this._err(res, next, e); }
  };
}
